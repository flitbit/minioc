...
Object.defineProperties(Container.prototype, {
		...
		value: function(it, val) {
			...
			if (typeof name !== 'undefined') {
				if (name === 'container' || name === 'root') {
					throw new Error('Invalid operation; `'.concat(name, '` is reserved.'));
				}
				var reg = this._reg
				, current = reg[name]
				;
	...
	fulfill: {
		value: function(id, fn, options) {
			...
			;
			this._unfulfilled[recorded] = reg = new Registration(id, this);
			reg.as.factory(fn, options);
	...
	checkUnfulfilled: {
		value: function() {
			var deps = []
			for (var k in this._unfulfilled) {
				var _deps = this._unfulfilled[k]._value._deps;
				for (var d in _deps) {
					var dep = _deps[d].name;
					if (!this.has(dep)) {
						deps.push(dep);
					}
				}
			...
			if (deps.length) {
				throw new Error('Unfulfilled dependencies: `'.concat(deps, '`'));
			}
		}
	}
...
Object.defineProperties(Container, {
	...
	fulfill: {
		value: function(it, val, options) {
			return __root.fulfill(it, val, options);
		},
		enumerable: true
	},
	...
	checkUnfulfilled: {
		value: function() {
			return __root.checkUnfulfilled();
		},
		enumerable: true
	}
});
